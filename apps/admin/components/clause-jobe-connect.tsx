...
const mockJobPositions: JobPosition[] = [
  { id: "1", name: "Software Engineer", department: "Engineering", division: "Technology", organization: "Tech Corp" },
  { id: "2", name: "Product Manager", department: "Product", division: "Technology", organization: "Tech Corp" },
  { id: "3", name: "QA Engineer", department: "Engineering", division: "Technology", organization: "Tech Corp" },
  { id: "4", name: "HR Manager", department: "Human Resources", division: "Operations", organization: "Tech Corp" },
  { id: "5", name: "Finance Analyst", department: "Finance", division: "Operations", organization: "Tech Corp" },
  { id: "6", name: "Marketing Specialist", department: "Marketing", division: "Business", organization: "Tech Corp" },
  { id: "7", name: "Sales Representative", department: "Sales", division: "Business", organization: "Tech Corp" },
  { id: "8", name: "DevOps Engineer", department: "Engineering", division: "Technology", organization: "Tech Corp" },
]
    ...
    clauses: [
      {
        ...
        description: "Guidelines for collecting personal information from users and employees.",
      },
      {
        id: "1-2",
        title: "Data Storage Security",
        description: "Requirements for secure storage and encryption of sensitive data.",
      },
      {
        id: "1-3",
        title: "Data Access Controls",
        description: "Procedures for controlling access to confidential information.",
      },
    ],
  },
  {
    ...
    clauses: [
      { id: "2-1", title: "Password Policy", description: "Standards for creating and maintaining secure passwords." },
      {
        id: "2-2",
        title: "Network Security",
        description: "Protocols for maintaining secure network connections and VPN usage.",
      },
      {
        id: "2-3",
        title: "Incident Response",
        description: "Procedures for responding to security breaches and incidents.",
      },
    ],
  },
  {
    ...
    clauses: [
      {
        ...
        description: "Adherence to industry regulations and legal requirements.",
      },
      { id: "3-2", title: "Audit Procedures", description: "Internal and external audit processes and requirements." },
      {
        id: "3-3",
...
export default function ClauseJobConnect() {
  ...
  const [selectedClause, setSelectedClause] = useState<PolicyClause | null>(null)
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Comprehensive policy framework for data protection, security, and compliance requirements.
        </p>
      </div>
                      ...
                      <Sheet open={isDrawerOpen} onOpenChange={setIsDrawerOpen}>
                        ...
                        <SheetContent className="w-[600px] sm:max-w-[600px]">
                          <SheetHeader>
                            ...
                            <p className="text-sm text-muted-foreground">{selectedClause?.title}</p>
                          </SheetHeader>

                          <div className="flex justify-between items-center mt-4 mb-2">
                            <div className="text-sm text-muted-foreground">
                              {Object.keys(selectedPositions).length} positions selected
                            </div>
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  const allDivs = allOrgs.flatMap((org) =>
                                    Object.keys(groupedPositions[org]).map((div) => `${org}-${div}`),
                                  )
                                  const allDepts = allOrgs.flatMap((org) =>
                                    Object.entries(groupedPositions[org]).flatMap(([div, depts]) =>
                                      Object.keys(depts).map((dept) => `${org}-${div}-${dept}`),
                                    ),
                                  )

                                  setExpandedOrganizations(Object.fromEntries(allOrgs.map((org) => [org, true])))
                                  setExpandedDivisions(Object.fromEntries(allDivs.map((div) => [div, true])))
                                  setExpandedDepartments(Object.fromEntries(allDepts.map((dept) => [dept, true])))
                                }}
                              >
                                Expand All
                              </Button>
                          ...
                          <div className="mt-6 space-y-4 max-h-[calc(100vh-120px)] overflow-y-auto">
                            {Object.entries(groupedPositions).map(([organization, divisions]) => {
                              const isOrgExpanded = expandedOrganizations[organization]
                              const divisionCount = Object.keys(divisions).length
                              const totalPositions = Object.values(divisions).reduce(
                                (acc, depts) =>
                                  acc +
                                  Object.values(depts).reduce((deptAcc, positions) => deptAcc + positions.length, 0),
                                0,
                              )
                              ...
                              return (
                                <Collapsible
                                  key={organization}
                                  open={isOrgExpanded}
                                  onOpenChange={() => toggleOrganization(organization)}
                                >
                                  ...
                                  <CollapsibleContent className="ml-6 mt-2 space-y-3">
                                    {Object.entries(divisions).map(([division, departments]) => {
                                      ...
                                      const divPositions = Object.values(departments).reduce(
                                      ...
                                      return (
                                        <Collapsible
                                          key={division}
                                          open={isDivExpanded}
                                          onOpenChange={() => toggleDivision(organization, division)}
                                        >
                                            ...
                                            {Object.entries(departments).map(([department, positions]) => {
                                              ...
                                              return (
                                                <Collapsible
                                                  ...
                                                  open={isDeptExpanded}
                                                  onOpenChange={() =>
                                                    toggleDepartment(organization, division, department)
                                                  }
                                                >
                                                  ...
                                                  <CollapsibleContent className="ml-4 mt-2 space-y-2">
                                                    {positions.map((position) => (
                                                      ...
                                                      >
                                                        <Checkbox
                                                          id={position.id}
                                                          checked={position.id in selectedPositions}
                                                          onCheckedChange={(checked) =>
                                                            handlePositionToggle(position.id, checked as boolean)
                                                          }
                                                        />
                                                        <label
                                                          htmlFor={position.id}
                                                        ...
                                                        {position.id in selectedPositions && (
                                                          <Select
                                                            ...
                                                            onValueChange={(value) =>
                                                              handleActionTypeChange(position.id, value)
                                                            }
                                                          >
                          ...
                          <div className="mt-6 pt-4 border-t">
                            <div className="flex justify-between">
                              ...
                              <Button onClick={() => setIsDrawerOpen(false)}>Save Connections</Button>
                            </div>
                          </div>
                        </SheetContent>
                      </Sheet>
