    ...
    <DialogContent className="sm:max-w-[600px] lg:max-w-[1000px] w-full overflow-y-auto h-[80vh]">
      ...
      >
        ...
        {selectedClauseJobPosition ? (
          <RatingForm selectedClauseJobPositionId={selectedClauseJobPosition?.id} position_name={selectedClauseJobPosition?.job_position.name} /> 
        ) : <ResizablePanel defaultSize={40} minSize={35}><div className="flex flex-col gap-4 h-full p-6 overflow-auto min-h-[600px]"> Ажлын байр сонгоно уу </div></ResizablePanel>}
      </ResizablePanelGroup>
...
function RatingForm({
  ...
  const [rating, setRating] = useState<Partial<Rating>>({ score: 1, description: null })
  ...
  return <ResizablePanel defaultSize={40} minSize={35}>
    <div className="flex flex-col gap-4 h-full p-6 overflow-auto">
      ...
      >
        ...
        {isLoading ? <div> Ачааллаж байна</div> : <div>
